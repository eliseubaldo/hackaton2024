...
export default class MainScene extends Phaser.Scene {
	...
	create() {
		...
		const problemText = this.add.text(185, 155, mathQuestion, this.mathQuestionProperties);
	...
	createCubes() {
		...
		for (let index = 0; index < 10; index++) {
			const cube: Phaser.Physics.Arcade.Sprite = this.cubeGroup.create(startPos.x, startPos.y, 'cubes', index);
			cube.setScale(2);
			cube.setOrigin(0);
	...
	setColliders() {
		this.physics.add.collider(this.cindy, this.cubeGroup, (cindy, cube)=>{ this.hitCube(cube)}, null, this);
	}
